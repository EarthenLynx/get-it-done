...
const handleWriteTodo = (req, res) => {
	...
	model
		...
		.catch(err =>
			res
				.status(500)
				.send({ msg: `An error occured while writing todo: ${err}` })
		);
};
...
const handleGetTodos = (req, res) => {
	model
		.getTodo()
		.then(data =>
			res.send({ msg: 'Successfully fetched todos from database', data })
		)
		.catch(err =>
			res
				.status(500)
...
const handleGetTodoById = (req, res) => {
	...
	model
		.getTodoById(id)
		.then(data =>
			res.send({ msg: 'Successfully fetched todo from database', data })
		)
		.catch(err =>
			res
				.status(500)
